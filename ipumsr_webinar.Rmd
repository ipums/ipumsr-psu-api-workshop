...
???
Let's say we want to visualize the trend in migration in Puerto Rico between 
2015 and 2019. Looking at the value labels for the variable MIGRATE1, it looks 
like values 0 and 9 are effectively missing, a value of 1 indicates that the 
person lives in the same house as a year ago, and values 2 through 4 indicate 
that they lived somewhere else a year ago.

If we just want to create a simple TRUE-FALSE flag for whether a person moved 
in the past year, we can use an approach like this the `case_when()` function 
from the dplyr package, setting values of 0 and 9 to missing, a value of 1 to 
FALSE, and values 2 through 4 to TRUE.
...
???
Now, we can use that nice, collapsed education variable that Kara created to 
...
.strong[`8a.` API use case 1: Revise your extract]
...
[Describe the code, including how the first few functions return the extract 
object, and `download_extract()` returns the ddi path, making for a 
...
The code on this and the next few slides shows one way to accomplish that. The
first thing you'll notice is that we are checking for the existence of a DDI
codebook file that isn't named with a collection and extract number, such as USA
14 from the previous slide, because if you are sharing this with someone else,
you don't know what the extract number of their extract will be. As you'll see,
if this DDI codebook file doesn't exist in the working directory, this code will
submit a new extract request, download the extract files, and rename them to
...
definition from the JSON file that you created with the `save_extract_as_json()` 
function.
...
the DDI filename using the `basename()` function on the full file path that's
...
to false and this code that downloads the data will be skipped.

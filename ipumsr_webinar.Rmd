...
if (!require("ipumsr"))
  stop(
    "Reading IPUMS data into R requires 
    the ipumsr package. It can be installed
...
- Set the working directory to the folder with these files (or change the path below).
...
- Luckily ipumsr provides helpers that allow you to use information from both 
  the value and label
...
```{r echo=FALSE, fig.alt="Screenshot from the IPUMS USA homepage with an arrow pointing to the 'Geography and GIS' link in the left sidebar."}

...
.names[Derek Burk, PhD, Sociology]


--


.names[Dan Ehrlich, MA, Anthropology]
...
.names[Kara Fisher, MA, Public Health]
...
<div style= "background-image: url("logos/usa.jpg");">
background-size: cover;
...
![IPUMS NHGIS project logo](logos/cps.jpg)

- Current Population Survey **microdata** from 1962 to the present.

- Monthly surveys and supplements.
...
![IPUMS NHGIS project logo](logos/dhs.jpg)

- Demographic and Health Surveys provide integrated **microdata** for analysis across time and space from the 1980s to the present.

--

- Surveys in this project usually come from countries without infrastructure for a complete census. 
--
...
- Health survey data from the National Health Interview Survey from the 1960s to the present and the Medical Expenditure Panel Survey from 1996.
...
![IPUMS NHGIS project logo](logos/highered.jpg)

- Survey data on the science and engineering workforce in the U.S. from 1993 to the present.
...
- **Tabular** U.S. Census data and GIS **shapefiles** from 1790 to the present.

--

- Shapefiles integrated across time.
---


![IPUMS Terra project logo](logos/terra.jpg)

- Integrated data on population and the environment from 1960 to the present.

--

- Tools for creating **summaries** of international census data and connecting to geographic data
...
```{r eval=FALSE}
last_extract <- get_recent_extracts_info_list("usa", 1)[[1]]
```
...
```{r eval=FALSE}
last_extract <- get_extract_info("usa:33")
```
...
# Get a tibble of info on recent extracts
...
```{r echo=FALSE}
readRDS("recent_extracts_info.rds")
```
...
```{r eval=FALSE}
define_extract_from_json("my_extract.json", "usa")
```
